...
- [PDF решения](public/solution.pdf)
- [Презентация решения — 11 слайдов](мвп/public/solution.pptx)
- [Презентация решения — 11 слайдов](public/solution.pptx)
...
Основной режим — **Cloudflare webhook**. Бот работает на сервере сайта, события появляются автоматически. Ноутбук можно выключить.

Управление с любого ноутбука:

1. Откройте сайт и нажмите «Вход команды», введите существующий ключ команды.
2. Нажмите «Бот» в верхней панели: статус, включение, пауза, приветствие и ссылка менеджера.
3. «Скачать CSV» выгружает реальные события с обезличенным userKey. Повторный импорт сохраняет совместимость с текущей воронкой.

Токен хранится только в Cloudflare secret, не в браузере. Webhook защищён отдельным секретом. На паузе очередь Telegram сохраняется; включение не удаляет ожидающие события. Start и payment_click сохраняются по устойчивым ID, кнопка покупки сохраняет источник именно своего меню.

Резервный режим — Python polling:

```bash
cp bot/.env.example bot/.env
# Укажите TELEGRAM_BOT_TOKEN из BotFather; .env не добавлять в Git.
pnpm bot:start
```

Нужен Python 3.10+ и включённый компьютер. Polling и webhook одновременно не работают: перед резервным запуском поставьте серверного бота на паузу. Launcher не удаляет webhook самостоятельно. Резервный CSV импортируется в «Воронку» вручную.

`pnpm bot:check` проверяет getMe/getWebhookInfo без отправки сообщений и получения updates. `scripts/setup_telegram_cloudflare.py` переносит токен из bot/.env в серверный secret через stdin и создаёт webhook secret; значения не выводятся.

Подробности: [docs/BOT_CONTROL.md](docs/BOT_CONTROL.md).
...
Funnel не меняет календарную схему. Для серверного бота добавлена миграция `0001_telegram_runtime.sql`: настройки/меню и журнал доставки. Финансовые события остаются в существующем хранилище. Production Worker/D1 и account subdomain оставлены существующими: они уже обслуживают выпущенные /r-ссылки. Старые слова в физическом https://demo-komandy-postupili.postupili-demo.workers.dev относятся к сохранённой инфраструктуре, название команды — «Наступашки».

## Структура проекта

- app — страницы и API
- components — Календарь, Воронка, Результаты
- lib — атрибуция, bot mapping, оплаты, reporting
- data — historical и demo
- scripts и bot — проверки, deploy, Python polling
- docs — методология и приёмка
- public — PDF и презентация
...
pnpm lint
pnpm build
pnpm bot:test  # после установки зависимостей бота в bot/.venv
```

56 engine/feature tests проверяют деньги, placement attribution, mapping, preview, refund, фильтры и CSV. Повторно пройдены 45 API / 18 public smoke; итог текущей доработки — в docs/POLISHING.md. Telegram вручную проверяет владелец после добавления токена. [Методология](docs/SOLUTION.md), [приёмка Funnel](docs/ACCEPTANCE_FUNNEL.md).
