...
import type{Workspace,Options,Placement,Touch,Payment}from '../lib/types';
...
test('No publication after purchase; no guessed organic source',()=>{const d=fixture();d.placements=[place('future','2026-09-11T00:00:00Z')];const r=attribute(d,'historical',opts);assert.equal(r[0].basis,'unknown');assert.equal(r[0].revenue,1000);});
test('Strict course matching and opt-in family coefficient',()=>{const d=fixture();const p=place('family');p.match='family_candidate';d.placements=[p];assert.equal(attribute(d,'historical',opts)[0].basis,'unknown');d.placements.push(place('exact'));const r=attribute(d,'historical',{...opts,family:true});assert.ok(Math.abs(r.find(a=>a.campaignId==='exact')!.weight-2/3)<1e-9);});
test('Reposts do not multiply campaign weight',()=>{const d=fixture();const a=place('a'),duplicate={...a,id:'repost'},b=place('b');d.placements=[a,duplicate,b];const r=attribute(d,'historical',{...opts,model:'linear'});assert.equal(r.length,2);assert.equal(r.find(v=>v.campaignId==='a')!.weight,0.5);});
test('Only same-visitor link visits are eligible; anonymous requests ignored',()=>{const d=fixture('demo');d.placements=[place('a',undefined,'demo'),place('b',undefined,'demo')];d.events=[{id:'e1',mode:'demo',userId:'u',placementId:'a',type:'visit',at:'2026-09-09T15:00:00Z'},{id:'e2',mode:'demo',userId:'other',placementId:'b',type:'visit',at:'2026-09-10T11:00:00Z'},{id:'e3',mode:'demo',userId:'u',placementId:'b',type:'click',at:'2026-09-10T11:00:00Z'}];assert.equal(attribute(d,'demo',{...opts,model:'last'})[0].campaignId,'a');});
test('First and last touch choose different eligible campaigns',()=>{const d=fixture('demo');d.placements=[place('a',undefined,'demo'),place('b',undefined,'demo')];d.events=['a','b'].map((placementId,i)=>({id:String(i),mode:'demo',userId:'u',placementId,type:'visit',at:i?'2026-09-10T11:00:00Z':'2026-09-09T12:00:00Z'}));assert.equal(attribute(d,'demo',{...opts,model:'first'})[0].campaignId,'a');assert.equal(attribute(d,'demo',{...opts,model:'last'})[0].campaignId,'b');});
test('Ad ROAS and full-marketing ROMI use explicit course economics',()=>{const d=fixture('demo');d.placements=[place('a',undefined,'demo')];d.events=[{id:'e',mode:'demo',userId:'u',placementId:'a',type:'visit',at:'2026-09-09T12:00:00Z'}];d.campaigns=[{id:'a',mode:'demo',name:'A',goal:'test',owner:'',extraCost:0,estimatedAdCost:null}];d.economics=[{id:'eco',mode:'demo',course:'ML про',deliveryPercent:40,feePercent:0,perSaleCost:0}];const s={...defaults('demo'),from:'2026-09-01',to:'2026-09-10',asOf:'2026-09-30'};let g=groupReport(report(d,s,opts,0),'campaign')[0];assert.equal(g.roas,null);d.placements[0].cost=0;g=groupReport(report(d,s,opts,0),'campaign')[0];assert.equal(g.romi,null);d.placements[0].cost=100;g=groupReport(report(d,s,opts,0),'campaign')[0];assert.equal(g.roas,10);assert.equal(g.romi,500);});
test('Naive purchase clock is adjustable without changing zoned post time',()=>{assert.equal(millis('2026-09-10T12:00:00',3),millis('2026-09-10T09:00:00Z'));assert.equal(millis('2026-09-10T12:00:00Z',3),millis('2026-09-10T12:00:00Z'));});
test('Refunded rows are not treated as paid conversions',()=>{const d=fixture();d.payments[0].status='refunded';const r=attribute(d,'historical',opts);assert.equal(r.length,1);assert.equal(r[0].revenue,0);assert.equal(r[0].refunds,1000);});
test('CSV quotes text and neutralises spreadsheet formula cells',()=>{assert.ok(csv([{a:'=1+1',b:'x,y"z'}]).includes("'=1+1"));assert.ok(csv([{a:'x,y"z'}]).includes('""'));});
test('All source purchases conserve amount and weight for every model/window/clock',()=>{
